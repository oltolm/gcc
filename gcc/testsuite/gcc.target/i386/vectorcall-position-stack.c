...
/* The final vector argument must be copied to a stack slot and passed by
   address rather than assigned an XMM register.  The address goes after
   six 8-byte home slots, even though the caller is not vectorcall.  */
/* { dg-final { scan-assembler {lea[ \t]+rax, [0-9]+\[rsp\]} { target x86_64-*-mingw* } } } */
/* { dg-final { scan-assembler {mov[ \t]+QWORD PTR 48\[rsp\], rax} { target x86_64-*-mingw* } } } */
